<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom:10px;">
			<el-pagination layout="total,sizes,prev, pager, next" @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[10,15, 20, 300, 400]" :page-size="pagesize" :total="total" style="float:right;">
			</el-pagination>
		</el-col>
...
export default {
	...
	methods: {
        ...
        formattertype(row){
            return row.type == 1 ? '分润金' : row.type == 2 ? '返利盘溢出或舍弃的值' : '3用户的返利无订单可返';
        },
		...
		getUsers() {
			...
			// 	NProgress.done();
			// });
			request.get(config.api.help.syslog,para)
				.then((res) => {
					this.listLoading = false;
					NProgress.done();
